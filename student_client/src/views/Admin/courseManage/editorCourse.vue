...
export default {
  ...
  created() {
    ...
    axios.get('http://localhost:8085/course/findById/' + this.ruleForm.cid).then(function (resp) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post("http://localhost:8085/course/updateCourse", this.ruleForm).then(function (resp) {
            if (resp.data === true) {
              that.$message({
                showClose: true,
                message: '编辑成功',
